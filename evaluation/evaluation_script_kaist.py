"""Evaluate MR-2 metric on KAIST
    This Python script is modified from MLPD [ https://github.com/sejong-rcv/MLPD-Multi-Label-Pedestrian-Detection/blob/main/evaluation_script/ ]
    COCOAPI is adopted. [cocoapi](https://github.com/cocodataset/cocoapi)
"""
...
from wccnet.utils import setup_logger
...
class KAISTPedEval(COCOeval):
    ...
    def draw_figure(ax, eval_results, methods, colors):
        ...
        ax.set_ylim(0.01, 1)
        ax.set_xlim(2e-4, 50)
        ax.set_ylabel(r'$\mathrm{MR}^{-2}$',fontdict=font)
        ax.set_xlabel('FPPI',fontdict=font)
...
class KAISTParams(Params):
    ...
    def setDetParams(self):
        ...
        # The HtRng settings follow MLPD in [ https://github.com/sejong-rcv/MLPD-Multi-Label-Pedestrian-Detection/blob/main/evaluation_script/evaluation_script.py ]
...
@logger.catch
def evaluate(test_annotation_file: str, user_submission_file: str, phase_codename: str = 'Multispectral', setup_exp:int=0):
    ...
    """
    logger.info(f'Loading from {user_submission_file}')
    print(f'Loading from {user_submission_file}')
    ...
    eval_result['all'].evaluate(setup_exp)
    eval_result['all'].accumulate()
    MR_all = eval_result['all'].summarize(setup_exp)

    eval_result['day'].params.imgIds = imgIds[:1455]
    eval_result['day'].evaluate(setup_exp)
    eval_result['day'].accumulate()
    MR_day = eval_result['day'].summarize(setup_exp)

    eval_result['night'].params.imgIds = imgIds[1455:]
    eval_result['night'].evaluate(setup_exp)
    eval_result['night'].accumulate()
    MR_night = eval_result['night'].summarize(setup_exp)
...
def draw_all(eval_results, filename='figure.jpg'):
    ...
    axes[2].set_title('Night',fontdict=font)
...
def draw_benchmark(eval_results, filename='figure.jpg'):
    """Draw all results in a single figure as Miss rate versus false positive per-image (FPPI) curve
    Parameters
    ----------
    eval_results: List of Dict
    ...
    eval_results_all = [res['all'].eval for res in eval_results]
    KAISTPedEval.draw_figure(axes, eval_results_all, methods, colors)
    plt.savefig(filename,bbox_inches = 'tight',pad_inches=0.1)
    
if __name__ == "__main__":
    # Annotation File
    annFile = 'evaluation/KAIST_annotation.json'
    # Prediction Result Files
    rstFiles = [
            'evaluation/kaist/ARCNN_result.txt',
            'evaluation/kaist/CIAN_result.txt',
            'evaluation/kaist/MSDS_result.txt',
            'evaluation/kaist/MLPD_result.txt',
            'evaluation/kaist/MBNet_result.txt',
            'evaluation/kaist/WCCNets/WCCNet_result.txt',
            'evaluation/kaist/WCCNets/WCCNet-L_result.txt',
            'evaluation/kaist/WCCNets/WCCNet-S_result.txt',
            'evaluation/kaist/WCCNets/WCCNet-XS_result.txt',
        ]

    # 0 for reasonable setting, 3 for all-dataset setting
    all_results = []
    for setup_exp,exp_name in zip([0,3],['reasonable','allDataset']):
        print(f'Start eval exp by {exp_name} setting...')
        evalFig = f'evaluation/KAIST_Benchmark_{exp_name}Setting.pdf'
        save_dir = 'evaluation/'
        setup_logger(save_dir, distributed_rank=0, filename=f"KAIST_Benchmark_{exp_name}Setting.txt", mode="o")
        phase = "Multispectral"
        
        results = [
            evaluate(annFile, rstFile, phase, setup_exp) 
            for rstFile in rstFiles
            ]
        # Sort results by MR_all
        results = sorted(results, key=lambda x: x['all'].summarize(0), reverse=True)
        all_results.append(results)
        draw_benchmark(results, filename=evalFig)
        logger.remove()
